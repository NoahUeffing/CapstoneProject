...
// This file creates the Axemen Hockey page with tabs for
...
Future<Roster> fetchRoster() async {
  ...
  final response = await http.get(api + '/mhockeyPlayers');
...
Future<Schedule> fetchSchedule() async {
  ...
  final response = await http.get(api + '/mhockeyGames');
  if (response.statusCode == 200) {
    // If the server did return a 200 OK response,
    // then parse the JSON.
...
Future<Standings> fetchStandings() async {
  ...
  final response = await http.get(api + '/mhockeyTeams');
  if (response.statusCode == 200) {
    // If the server did return a 200 OK response,
    // then parse the JSON.
...
// Create a list of Acadia hockey players
class Roster {
...
// Create a list of Acadia hockey games
class Schedule {
...
// Create a list of AUS hockey teams with stats
class Standings {
...
class AxemenHockeyState extends State<AxemenHockey> {
  ...
  final urlList = [
    ...
    'https://www.goxgo.ca/sports/mice/2020-21/roster',
  ];
          ...
          appBar: AppBar(
            ...
            title: RichText(
              text: TextSpan(
                children: [
                  WidgetSpan(
                    child: Icon(globals.hockeyIcon,
                        size: globals.titleSize, color: globals.titleColor),
                  ),
                  TextSpan(
                      text: " " + globals.menHockeyTitle,
                      style: globals.titleFont),
                ],
              ),
            ),
