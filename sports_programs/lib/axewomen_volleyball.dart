...
// This file creates the Axewomen Volleyball page with tabs for
...
Future<Roster> fetchRoster() async {
  ...
  final response = await http.get(api + '/wvolleyPlayers');
...
Future<Schedule> fetchSchedule() async {
  ...
  final response = await http.get(api + '/wvolleyGames');
  if (response.statusCode == 200) {
    // If the server did return a 200 OK response,
    // then parse the JSON.
...
Future<Standings> fetchStandings() async {
  ...
  final response = await http.get(api + '/wvolleyTeams');
  if (response.statusCode == 200) {
    // If the server did return a 200 OK response,
    // then parse the JSON.
...
// Create a list of Acadia volleyball players
class Roster {
...
// Create a list of Acadia volleyball games
class Schedule {
...
// Create a list of AUS volleyball teams with stats
class Standings {
...
class AxewomenVolleyballState extends State<AxewomenVolleyball> {
  ...
  final urlList = [
    'https://daltigers.ca/sports/wvball/2020-21/roster',
    'https://www.goseahawks.ca/sports/wvball/2020-21/roster',
    'https://www.umoncton.ca/umcm-sports/node/189',
    'https://goredsgo.ca/sports/wvball/2020-21/roster',
    'https://www.smuhuskies.ca/sports/wvball/2020-21/roster',
    'https://www.gotommies.ca/sports/wvball/2020-21/roster',
    'https://www.goxgo.ca/sports/wvball/2012-13/roster',
  ];
  ...
  Widget _buildList() {
    return Theme(
        ...
        child: TabBarView(children: [
          ...
          WebView(
            initialUrl:
                'https://www.acadiaathletics.ca/sports/wvball/2020-21/roster',
            javascriptMode: JavascriptMode.unrestricted,
          ),
          ...
          appBar: AppBar(
            ...
            title: RichText(
              text: TextSpan(
                children: [
                  WidgetSpan(
                    child: Icon(globals.volleyIcon,
                        size: globals.titleSize, color: globals.titleColor),
                  ),
                  TextSpan(
                      text: " " + globals.womenVolleyTitle,
                      style: globals.titleFont),
                ],
              ),
            ),
