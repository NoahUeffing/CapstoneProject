...
// This file creates the Axemen Soccer page with tabs for
...
Future<Roster> fetchRoster() async {
  ...
  final response = await http.get(api + '/msoccerPlayers');
...
Future<Schedule> fetchSchedule() async {
  ...
  final response = await http.get(api + '/msoccerGames');
  if (response.statusCode == 200) {
    // If the server did return a 200 OK response,
    // then parse the JSON.
...
Future<Standings> fetchStandings() async {
  ...
  final response = await http.get(api + '/msoccerTeams');
  if (response.statusCode == 200) {
    // If the server did return a 200 OK response,
    // then parse the JSON.
...
// Create a list of Acadia soccer players
class Roster {
...
// Create a list of Acadia soccer games
class Schedule {
...
// Create a list of AUS soccer teams with stats
class Standings {
...
class Player {
  ...
  Player({this.id, this.no, this.name, this.pos, this.yr, this.town});
...
class AxemenSoccerState extends State<AxemenSoccer> {
  ...
  final urlList = [
    'https://gocapersgo.ca/sports/msoc/2020-21/roster',
    'https://daltigers.ca/sports/msoc/2020-21/roster',
    'https://www.goseahawks.ca/sports/msoc/2020-21/roster',
    'https://www.umoncton.ca/umcm-sports/node/188',
    'https://mountiepride.ca/sports/msoc/2020-21/roster',
    'https://goredsgo.ca/sports/msoc/2020-21/roster',
    'https://gopanthersgo.ca/sports/msoc/2020-21/roster',
    'https://www.smuhuskies.ca/sports/msoc/2020-21/roster',
    'https://www.gotommies.ca/sports/msoc/2020-21/roster',
    'https://www.goxgo.ca/sports/msoc/2020-21/roster',
  ];
  ...
  Widget _buildList() {
    return Theme(
        ...
        child: TabBarView(children: [
          ...
          WebView(
            initialUrl:
                'https://www.acadiaathletics.ca/sports/msoc/2019-20/roster',
            javascriptMode: JavascriptMode.unrestricted,
          ),
          ...
          appBar: AppBar(
            ...
            title: RichText(
              text: TextSpan(
                children: [
                  WidgetSpan(
                    child: Icon(globals.soccerIcon,
                        size: globals.titleSize, color: globals.titleColor),
                  ),
                  TextSpan(
                      text: " " + globals.menSoccerTitle,
                      style: globals.titleFont),
                ],
              ),
            ),
