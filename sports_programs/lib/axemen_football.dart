...
// This file creates the Axemen Football page with tabs for
...
Future<Roster> fetchRoster() async {
  ...
  final response = await http.get(api + '/mfootballPlayers');
...
Future<Schedule> fetchSchedule() async {
  ...
  final response = await http.get(api + '/mfootballGames');
  if (response.statusCode == 200) {
    // If the server did return a 200 OK response,
    // then parse the JSON.
...
Future<Standings> fetchStandings() async {
  ...
  final response = await http.get(api + '/mfootballTeams');
  if (response.statusCode == 200) {
    // If the server did return a 200 OK response,
    // then parse the JSON.
...
// Create a list of Acadia football players
class Roster {
...
// Create a list of Acadia football games
class Schedule {
...
// Create a list of football teams with stats
class Standings {
...
class Game {
  ...
  final String status;
  Game({this.id, this.date, this.homeAway, this.opponent, this.status});
...
class AxemenFootballState extends State<AxemenFootball> {
  ...
  final urlList = [
    'https://gaiters.ca/sports/football/roster',
    'https://mountiepride.ca/sports/fball/2020-21/roster',
    'https://www.smuhuskies.ca/sports/fball/2020-21/roster',
    'https://www.goxgo.ca/sports/fball/2020-21/roster',
  ];

  // colours for opponent roster buttons
  final colorsList = [
    globals.bishopPurple,
    globals.mtaGold,
    globals.smuPink,
    globals.stfxBlue,
  ];
  ...
  Widget _buildList() {
    return Theme(
        ...
        child: TabBarView(children: [
          ...
          WebView(
            initialUrl:
                'https://www.acadiaathletics.ca/sports/fball/2019-20/roster',
            javascriptMode: JavascriptMode.unrestricted,
          ),
          ...
          appBar: AppBar(
            ...
            title: RichText(
              text: TextSpan(
                children: [
                  WidgetSpan(
                    child: Icon(globals.footballIcon,
                        size: globals.titleSize, color: globals.titleColor),
                  ),
                  TextSpan(
                      text: " " + globals.menFootballTitle,
                      style: globals.titleFont),
                ],
              ),
            ),
