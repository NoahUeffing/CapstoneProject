...
// This file creates the Axewomen Rugby page with tabs for
...
Future<Roster> fetchRoster() async {
  ...
  final response = await http.get(api + '/wrugbyPlayers');
...
Future<Schedule> fetchSchedule() async {
  ...
  final response = await http.get(api + '/wrugbyGames');
  if (response.statusCode == 200) {
    // If the server did return a 200 OK response,
    // then parse the JSON.
...
Future<Standings> fetchStandings() async {
  ...
  final response = await http.get(api + '/wrugbyTeams');
  if (response.statusCode == 200) {
    // If the server did return a 200 OK response,
    // then parse the JSON.
...
// Create a list of Acadia rugby players
class Roster {
...
// Create a list of Acadia rugby games
class Schedule {
...
// Create a list of AUS rugby teams with stats
class Standings {
...
class Player {
  ...
  Player({this.id, this.name, this.pos, this.yr, this.town});
...
class AxewomenRugbyState extends State<AxewomenRugby> {
  ...
  final urlList = [
    'https://gopanthersgo.ca/sports/wrugby/2020-21/roster',
    'https://www.smuhuskies.ca/sports/wrugby/2020-21/roster',
    'https://www.gotommies.ca/sports/wrugby/2020-21/roster',
    'https://www.goxgo.ca/sports/wrugby/2020-21/roster',
  ];
  ...
  Widget _buildList() {
    return Theme(
        ...
        child: TabBarView(children: [
          ...
          WebView(
            initialUrl:
                'https://www.acadiaathletics.ca/sports/wrugby/2019-20/roster',
            javascriptMode: JavascriptMode.unrestricted,
          ),
          ...
          appBar: AppBar(
            ...
            title: RichText(
              text: TextSpan(
                children: [
                  WidgetSpan(
                    child: Icon(globals.rugbyIcon,
                        size: globals.titleSize, color: globals.titleColor),
                  ),
                  TextSpan(
                      text: " " + globals.womenRugbyTitle,
                      style: globals.titleFont),
                ],
              ),
            ),
