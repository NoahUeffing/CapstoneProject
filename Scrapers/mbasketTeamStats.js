...
(async () => {
  try {
    ...
    await page.goto(
      `http://www.acadiaathletics.ca/sports/mbkb/2019-20/standings`
    );
    ...
    await page.waitForSelector(
      "#mainbody > div.full-standings.clearfix > div > div.overflow > div > table > tbody > tr > td.stats-field.points"
    );
    ...
    fs.writeFile(
      __dirname + "/../data/mbasketTeamStats.json",
      JSON.stringify(data),
      function (err) {
        if (err) throw err;
        console.log("Saved!");
